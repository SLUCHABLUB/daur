...
use crate::{Audio, Cell, CloneCell, Track};
...
pub(crate) struct Renderer {
    ...
    progress: CloneCell<Arc<Progress>>,

    // this is not part of progress since as it should it not reset
    /// This is set if ite audio should play.
...
impl Renderer {
    pub(crate) fn play_when_finished(&self, from: Instant, player: Player) {
        ...
        if let Some(audio) = self.progress.get().audio.get() {
            // check that the mastering thread has not started playing the audio
            if self.should_play.replace(None).is_none() {
                return;
            }

            main_player.play(audio, from);
        }
    }

    // TODO: inject into manager instead of cloning tracks
    ...
    pub(crate) fn restart(
        ...
        let old_progress = self.progress.replace(progress);
...
fn master(
    ...
    progress.audio.set(Some(audio));

    if let Some(should_play) = should_play.replace(None) {
        let Some(audio) = progress.audio.get() else {
            return;
        };

        should_play.player.play(audio, should_play.from);
    }
}
